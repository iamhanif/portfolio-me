...
const CaseStudy = () => {
  ...
  return (
    <section className="bg-white scroll-smooth dark:bg-gray-900 min-h-screen">
      <div className="flex max-w-7xl scroll-smooth mx-auto flex-col md:flex-row bg-white px-8 py-16 gap-5 dark:bg-gray-900">
        {/* Main Content: Project Details */}
        <div className="w-full md:w-4/6 px-4">
          <Link
            href="/project"
            className="text-gray-800 dark:text-lime-400 mb-4 inline-block"
          >
            <FaArrowCircleLeft className="inline mr-2" />
            Back to Projects
          </Link>
          <h2 className="text-lg md:text-4xl font-semibold text-center text-gray-800 dark:text-lime-400 mb-8">
            Case Study of {project.name}
          </h2>

          <motion.div
            animate={controls}
            className="mx-auto mb-8 hover:opacity-100 cursor-pointer transition-opacity duration-300 border dark:border-lime-400 rounded-lg p-2 md:p-16 shadow-lg opacity-95"
          ...
          <div className="max-w-4xl mx-auto mb-12">
            ...
            >
              <h3 className="text-2xl font-semibold text-gray-800 dark:text-lime-400 mb-4">
                Technologies Used
              </h3>
              <div className="flex flex-wrap gap-2 mb-8">
                {project.technologies.map((tech, index) => (
                  <span
                    key={index}
                    className="bg-lime-500 text-white px-3 py-1 rounded-full text-sm font-medium"
                  >
                    {tech}
                  </span>
                ))}
            ...
            >
              ...
              {project.link && (
                <Link
                  href={project.link}
                  className="text-lime-700 hover:text-lime-800 h-14 w-auto flex items-center"
                  target="_blank"
                >
        ...
        >
          <h3 className="text-xl font-semibold text-center text-gray-800 dark:text-lime-400 mb-8">
            Other Projects
          </h3>
          <div className="grid grid-cols-1 gap-8">
            {otherProjects.map((proj) => (
              <div
                key={proj.id}
                className="block group relative cursor-pointer overflow-hidden border dark:border-lime-400 h-48 rounded-lg shadow-lg"
              >
